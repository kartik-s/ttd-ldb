...
## How it works
`ttd-ldb` connects as a client to WinDbg's debug server, communicating with it using the `DbgEng` API. It starts as an empty process and does some setup. The final thing it does is to query the remote process for the address of `ldb_monitor`, the entry point to SBCL's low-level debugger, and call into it. This immediately triggers an access violation because the code is not present in the process.
